...
-- REWRITTEN 2026-09-18 (operator ruling). The old order was affiliate FIRST,
-- which meant a customer the AlterCPA bridge delivered stayed "affiliate"
-- forever — every later re-sale to that same person kept claiming affiliate
-- credit and the prediction team's revenue showed as 0,9% of the business when
-- it was actually more than half (measured: EUR 11.398 vs EUR 614.835 on the
-- same paid/delivered cohort). PREDICTION NOW WINS.
--
-- The operator's definitions, verbatim (2026-09-18):
--   "Pending is a lead that's never been changed status to cancelled, trashed
--    or paid. Prediction is a client who ordered before and we are re-calling
--    to make him buy again. Under pending/leads get only the new clients that
--    the affiliate gave us by making ads. Teleshop leads are something else
--    though, they are coming from the TV, not the affiliate."
--
-- So the three buckets — and there are only three, 'inbound' is retired:
--   prediction  the customer ORDERED BEFORE. Re-sale, regardless of where that
--               customer originally came from. Origin detail (teleshop / out /
--               leads / social) belongs in the Prediction Lists tab, not here.
--   affiliate   FIRST-EVER order that arrived through the AlterCPA bridge —
--               a new client the affiliate produced by running ads.
--   manual      first-ever order from anywhere else: teleshop (TV), walk-in,
--               social, operator-created. TV is NOT the affiliate.
--
-- "Ordered before" is ANY earlier order on the same phone regardless of its
-- outcome, per the operator's plain wording ("a client who ordered before").
-- Matching is last-8-digits, the CRM phone canon.
--
-- This is a pure RECLASSIFICATION: the same money, re-bucketed. Totals tie
-- exactly to the old ones (verified: EUR 1.225.581 both ways). Because the
-- channel is derived at query time, every historical report changes the moment
-- this lands — that is intended.
CREATE OR REPLACE FUNCTION public.order_channel(
  p_source_type        text,
  p_prediction_list_id uuid,
  p_is_affiliate       boolean,
  p_ordered_before     boolean
) RETURNS text
LANGUAGE sql IMMUTABLE PARALLEL SAFE AS $$
  SELECT CASE
    WHEN coalesce(p_ordered_before, false)
      THEN 'prediction'
    WHEN coalesce(p_is_affiliate, false) OR p_source_type IN ('altercpa','affiliate')
      THEN 'affiliate'
    ELSE 'manual'
  END
$$;
COMMENT ON FUNCTION public.order_channel(text, uuid, boolean, boolean) IS
  'Canonical order channel for P&L: prediction (customer ordered before) > affiliate (first-ever AlterCPA lead) > manual (everything else, incl. teleshop/TV). Operator ruling 2026-09-18.';

-- The 3-arg version is retired. Dropping it prevents a stale caller from
-- silently resolving to the OLD affiliate-first logic via overload matching.
DROP FUNCTION IF EXISTS public.order_channel(text, uuid, boolean);
...
AS $$
WITH first_order AS (
  -- Earliest order per customer across the WHOLE table — deliberately NOT
  -- date-filtered. The cohort below is windowed, but "did this person order
  -- before?" can only be answered against full history; scoping it to the
  -- range would relabel every returning customer as new at the range edge.
  SELECT right(regexp_replace(o.customer_phone, '[^0-9]', '', 'g'), 8) AS p8,
         min(o.created_at)                                          AS first_at
  FROM public.orders o
  WHERE o.customer_phone IS NOT NULL
    AND length(regexp_replace(o.customer_phone, '[^0-9]', '', 'g')) >= 8
...
base AS (
  ...
  SELECT o.id, o.status::text AS status, o.price, o.quantity, o.product_name,
         ...
         public.order_channel(o.source_type, o.prediction_list_id,
                              (o.cpa_webmaster_id IS NOT NULL
                                 OR o.external_source = 'altercpa'),
                              (f.first_at IS NOT NULL
                                 AND o.created_at > f.first_at)) AS channel
  FROM public.orders o
  LEFT JOIN first_order f
         ON f.p8 = right(regexp_replace(o.customer_phone, '[^0-9]', '', 'g'), 8)
...
REVOKE ALL ON FUNCTION public.order_channel(text, uuid, boolean, boolean) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.order_channel(text, uuid, boolean, boolean) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.order_channel(text, uuid, boolean, boolean) TO service_role;
